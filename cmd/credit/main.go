// credit.go — Запрос баллов WorkBuddy (все аккаунты + итог), JSON в stdout.
//
// Использование:
//
//	go run ./cmd/credit        # или скомпилированный ./credit
//
// Структура вывода:
//
...
//
// Логика интерфейсов и агрегации: POST codebuddy.cn/v2/billing/meter/get-user-resource, агрегируются все пакеты по
// полям Cycle*, TotalDosage как нижняя граница size.
package main
...
// packageRemainUsed См. billing.go:203-258
...
// printPretty Человекочитаемая сводка: четыре строки итога без детализации по аккаунтам.
func printPretty(accounts []accountResult, totalRemain, totalUsed, totalSize int64, okCount int) {
	...
	fmt.Printf("📊 Сводка баллов WorkBuddy\n")
	fmt.Printf("Аккаунты: %d/%d\n", withBalance, len(accounts))
	fmt.Printf("Всего: %d/%d\n", totalRemain, totalSize)
